...
def localization(localizer, fullImage, l_ios, l_iou, log):
    img = Image.fromarray(fullImage, 'RGB')
    img.save("py_local" + str(time.time())+".jpg")
    print("image shape : ", fullImage.shape)
    plateImage = []
    bbConf = 0.0
    plateType = ''
    lError = ''
    ...
    img.save("py_result" + str(time.time())+".jpg")
...
def inference_an_image_simple(fullImage, localizer, recognizer, lIos=0.8, lIou=0.3, rIos=0.8, rIou=0.3):
    img = Image.fromarray(fullImage, 'RGB')
    img.save("py_inference" + str(time.time())+".jpg")
    log = 1;
    # if debug:
    #     global config
    # if IF_LOGING:
    ...
    print("python argument image type : ", type(fullImage))
    ...
    if lError:
        ...
        result = plateString
        print("plate in python : ", plateString)
