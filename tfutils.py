...
#!!!!!!
#font_name = fm.FontProperties(fname='/home/qisens/Desktop/GULIM.ttc',size=100).get_name()
...
PLATE_HANGULS = ['ga', 'na', 'da', 'ra', 'ma', 'ba', 'sa', 'aa', 'ja', 'ha',
                 'goe', 'noe', 'doe', 'roe', 'moe', 'boe', 'soe', 'aoe', 'joe', 'hoe',
                 'go', 'no', 'do', 'ro', 'mo', 'bo', 'so', 'oo', 'jo', 'ho',
                 'gu', 'nu', 'du', 'ru', 'mu', 'bu', 'su', 'wu', 'ju', 'bae']
PLATE_YOUNG = '영'
...
def filter_bbox_ios(bboxes, ios_threshold):
    ...
    bboxes.sort(key=functools.cmp_to_key(bb.BBox.xcmp))
    left_idx = 0
...
def draw_bboxes(image, bboxes):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if 0:
            if bbox.conf < 0.4 :
                current_axis.text(xmin, ymin+ymax+(bbox.conf)*20, '({}){}({:.2f})'.format(i, bbox.name, bbox.conf), bbox={'facecolor': colors[i], 'alpha': 0.5})
            #elif bbox.conf > 0.3 :
            elif bbox.conf >= 0.4 and bbox.conf < 0.7 :
                current_axis.text(xmin, ymin-(bbox.conf)*20, '({}){}({:.2f})'.format(i, bbox.name, bbox.conf), bbox={'facecolor': colors[i], 'alpha': 0.5})
            else :
                current_axis.text(xmin, ymin, '({}){}({:.2f})'.format(i, bbox.name, bbox.conf), bbox={'facecolor': colors[i], 'alpha': 0.5})
        else:
            # if bbox.conf < 0.4:
