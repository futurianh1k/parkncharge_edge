...
//-------------------------------------------------------------------------------------------------------------------
					...
					result = inference_image(p_inference, name, localizer, recognizer);
					...
					if (PyUnicode_Check(result)) 
					{
						...
						if (temp_bytes != NULL) {
							...
							parkingSpot-> mPlateNumber = plate_str;

							//std::cout << "mPlateNumber : " << parkingSpot-> mPlateNumber << std::endl;
							//cv::Scalar black = CV_RGB(0,0,0);
							//cv::putText(frame, plate_str, cv::Point(parkingSpot->ROI.x, (parkingSpot->ROI.y + parkingSpot->ROI.height + 20)), cv::FONT_HERSHEY_PLAIN, 2.0, black, 2);
...
*******************************************************************************/
PyObject* inference_image(PyObject* p_inference, const char* img_path, PyObject* localizer, PyObject* recognizer)
{
	...
	if(p_inference)
	{
		...
		if(init_inference_image)
		{
            ...
            inference = PyObject_CallFunction(init_inference_image, "sOO", img_path, localizer, recognizer);
			if(inference)
			{
